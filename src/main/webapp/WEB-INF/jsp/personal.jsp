...
<body>
	...
	<div id="banner"
		style="width:100%;background-image:url('<%=basePath%>static/images/top_bg.png')">
		<img src="http://localhost:8080/finalDesign/static/images/top_bg.png" />
		<input value="<%=basePath%>" type="hidden" id="basePath"> <input
					...
					<div id="page-content">
						...
						<div class="container">
							<div class="row">
								<div class="col-md-4">
									<div class="box-item">
										<input type="hidden" class="blogId" value="${blog.id}" /> <img
											src="${blog.headImgUrl}" class="img-responsive blogImgs" />
										<div class="content">
											<h3>${blog.title}</h3>
											<p>${blog.headText}</p>
											<a href="<%=basePath%>blog/blogDetail/${blog.id}">Read
												More...</a> <br> <br> <span>${blog.updateTime}</span><br>
												
											<c:if test="${blog.visibility==1}">
												<span>本文仅个人可见</span>
											</c:if>
											<c:if test="${blog.visibility==0}">
												<span>本文公众可见</span>
											</c:if><br>
